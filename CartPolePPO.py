...
def play(model):
    ...
    state, info = env.reset()  # Resetting the environment
    for _ in range(200):
...
def train(policy_net, actor, critic, ppo_steps, ppo_clip, optimizer):
    for episode in range(1, MAX_EPISODES+1):
        policy_loss, value_loss, train_reward = train_one_episode(
            policy_net, actor, critic, ppo_steps, ppo_clip, optimizer, GAMMA
        )
...
def plot_rewards(train_rewards, test_rewards):
    # Create the figure once at the end of training
    plt.figure(figsize=(12, 8))
    plt.plot(test_rewards, label='Test Reward')
    plt.plot(train_rewards, label='Train Reward')
    plt.xlabel('Episode', fontsize=20)
    plt.ylabel('Reward', fontsize=20)
    plt.hlines(REWARD_THRESHOLD, 0, len(test_rewards), color='r')  # Threshold line
    plt.legend(loc='lower right')
    plt.grid()

    plt.show()  # Show the plot at the end
    plt.close()  # Close the plot to prevent multiple windows
